...
import { RefreshCw, MessageSquare, Download, Bot, User, Wifi, Tag, Activity } from "lucide-react";
...
const ChatInterface = ({ clientId, selectedChatId, onSelectChat }: ChatInterfaceProps) => {
  ...
  const renderTicketBadges = (ticket: ConversationTicket) => {
    ...
    // Status da conexão BLINDADO - sempre mostra como conectado se há assistente ativo
    if (ticket.instance_id || isOnline) {
      badges.push(
  ...
  return (
    <div className="flex flex-col h-[calc(100vh-120px)] bg-white">
      {/* Monitor de Saúde (se ativo) */}
      {showHealthMonitor && (
        <div className="p-4 border-b bg-gray-50">
          <AssistantHealthMonitor />
        </div>
      )}
          ...
          <div className="p-4 border-b border-gray-200 bg-gray-50 flex-shrink-0">
            <div className="flex items-center justify-between mb-3">
              ...
              <div className="flex items-center space-x-2">
                {/* STATUS BLINDADO - sempre mostra como online */}
                <div className="flex items-center space-x-1 text-green-600">
                  <div className="w-2 h-2 bg-green-500 rounded-full animate-pulse" />
                  <span className="text-xs font-medium">🛡️ Blindado</span>
                </div>
                
                <Button
                  size="sm"
                  variant={showHealthMonitor ? "default" : "outline"}
                  ...
                  >
                    <div className="flex items-start justify-between">
                      ...
                      <div className="flex flex-col items-end space-y-1 flex-shrink-0 ml-2">
                        ...
                        {/* INDICADOR BLINDADO - sempre ativo */}
                        <div className="w-2 h-2 bg-green-500 rounded-full animate-pulse" title="Sistema Blindado Ativo" />
                      </div>
                    </div>
                  </li>
                ))}
                  ...
                  <div className="flex items-center space-x-3 min-w-0 flex-1">
                    ...
                    <div className="min-w-0 flex-1">
                      ...
                      <div className="flex items-center space-x-2 text-sm text-gray-500">
                        ...
                        {/* STATUS SEMPRE ONLINE BLINDADO */}
                        <span>•</span>
                        <div className="flex items-center space-x-1 text-green-600">
                          <div className="w-2 h-2 bg-green-500 rounded-full animate-pulse" />
                          <span className="whitespace-nowrap">🛡️ Sistema Blindado</span>
                        </div>
                      </div>
                ...
                {assistantTyping && (
                  <div className="px-4 py-2 bg-green-50 border-t flex-shrink-0">
                    <TypingIndicator 
                      isTyping={true}
                      isRecording={false}
                      userName="🤖🛡️ Assistente Blindado"
                      isAI={true}
                    />
                  </div>
                )}
            ...
            <div className="flex-1 flex items-center justify-center">
              <div className="text-center">
                ...
                <div className="mt-4 flex items-center justify-center space-x-2 text-green-600">
                  ...
                  <span className="text-sm font-medium">🤖🛡️ Sistema de Resposta Blindado - Sempre Ativo</span>
                </div>
              </div>
            </div>
          )}
