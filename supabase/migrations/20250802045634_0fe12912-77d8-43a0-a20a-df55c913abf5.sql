-- Corrigir ticket atual: atribuir assistente da fila
UPDATE public.conversation_tickets 
SET assigned_assistant_id = (
  SELECT q.assistant_id 
  FROM public.queues q 
  WHERE q.id = conversation_tickets.assigned_queue_id 
  AND q.assistant_id IS NOT NULL
)
WHERE id = '732825f8-c1a3-4cc8-b6dc-caee0a611025'
AND assigned_assistant_id IS NULL;

-- Melhorar função upsert_conversation_ticket para sempre atribuir assistente quando fila for atribuída
...
BEGIN
  ...
  IF v_existing_ticket_id IS NOT NULL THEN
    ...
    ELSE
      -- Mesma instância - verificar se assistente está faltando
