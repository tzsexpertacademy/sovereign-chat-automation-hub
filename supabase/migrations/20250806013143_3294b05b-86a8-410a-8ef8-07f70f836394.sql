-- FASE 1: CORREÇÃO IMEDIATA DA FUNÇÃO RPC
-- Melhorar upsert_conversation_ticket para SEMPRE atribuir assistente quando fila for atribuída
-- MUDANÇAS SEGURAS: só adicionar lógica, não quebrar nada existente
...
DECLARE
  ...
  v_final_assistant_id UUID; -- NOVA VARIÁVEL PARA GARANTIR ASSISTENTE
BEGIN
  ...
  IF v_existing_ticket_id IS NOT NULL THEN
    ...
    IF v_existing_status IN ('closed', 'resolved') THEN
      ...
      -- NOVA LÓGICA: Buscar assistente da fila se a fila foi encontrada
      ...
      ELSE
        RAISE LOG '⚠️ [UPSERT-TICKET] REABERTURA: Nenhuma fila automática encontrada';
      END IF;
    ...
    ELSIF v_existing_instance_id != p_instance_id THEN
      ...
      ELSE
        RAISE LOG '⚠️ [UPSERT-TICKET] TRANSFERÊNCIA: Nenhuma fila automática encontrada';
      END IF;
    ...
    ELSE
      ...
      -- NOVA LÓGICA MELHORADA: Verificar se assistente está faltando
      ...
      IF v_queue_id IS NOT NULL AND v_assistant_id IS NULL THEN
        ...
        RAISE LOG '🔧 [UPSERT-TICKET] CORREÇÃO AUTOMÁTICA: Corrigindo assistente faltante: fila % -> assistente %', v_queue_id, v_assistant_id;
      END IF;
  ...
  ELSE
    ...
    ELSE
      RAISE LOG '⚠️ [UPSERT-TICKET] CRIAÇÃO: Nenhuma fila automática encontrada para instância: %', p_instance_id;
    END IF;
  ...
  -- NOVA CAMADA DE SEGURANÇA FINAL: Verificar se o ticket final tem fila mas não tem assistente
  SELECT ct.assigned_queue_id, ct.assigned_assistant_id 
  ...
  IF v_queue_id IS NOT NULL AND v_final_assistant_id IS NULL THEN
    ...
    IF v_final_assistant_id IS NOT NULL THEN
      ...
      RAISE LOG '🛡️ [UPSERT-TICKET] SEGURANÇA FINAL: Assistente corrigido para ticket %: fila % -> assistente %', 
        v_ticket_id, v_queue_id, v_final_assistant_id;
    ELSE
      RAISE LOG '❌ [UPSERT-TICKET] FALHA CRÍTICA: Fila % não tem assistente ativo!', v_queue_id;
    END IF;
  ELSIF v_queue_id IS NOT NULL AND v_final_assistant_id IS NOT NULL THEN
    RAISE LOG '✅ [UPSERT-TICKET] VERIFICAÇÃO FINAL OK: Ticket % tem fila % e assistente %', 
      v_ticket_id, v_queue_id, v_final_assistant_id;
  ELSIF v_queue_id IS NULL THEN
    RAISE LOG '⚠️ [UPSERT-TICKET] ATENÇÃO: Ticket % sem fila atribuída', v_ticket_id;
  END IF;
