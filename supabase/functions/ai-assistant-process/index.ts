  ...
  try {
    ...
    if (videoCommandMatch) {
      ...
      if (client?.business_token) {
        ...
        // Processar comando de vídeo SEGUINDO A MESMA LÓGICA DA IMAGEM
...
async function sendLibraryImageMessage(
  ...
  try {
    ...
    if (!response.ok) {
      const errorText = await response.text();
      console.error('❌ [SEND-LIBRARY-IMAGE] Erro na API:', errorText);
      throw new Error(`Falha no envio: ${response.status} - ${errorText}`);
    }
...
async function processVideoCommands(
  ...
  try {
    ...
    // ✅ REGEX PARA COMANDO DE VÍDEO: "video trigger"
...
async function getVideoFromLibrary(assistantId: string, videoTrigger: string): Promise<{ videoBase64: string, format: string } | null> {
  try {
    ...
    console.log('🔍 [VIDEO-LIBRARY] Dados do assistente raw:', {
      hasAdvancedSettings: !!assistantData?.advanced_settings,
      typeOfAdvancedSettings: typeof assistantData?.advanced_settings,
      rawAdvancedSettings: JSON.stringify(assistantData?.advanced_settings).substring(0, 200) + '...'
    });
    
    // 🎯 PARSER REFORÇADO PARA ESTRUTURA COMPLEX ANINHADA (IGUAL IMAGEM)
    let advancedSettings = assistantData?.advanced_settings || {};
    ...
    // STEP 2: ALGORITMO RECURSIVO PARA ENCONTRAR video_library
    let videoLibrary: any[] = [];
    
    function findVideoLibraryRecursively(obj: any, path: string = ''): any[] {
      if (!obj || typeof obj !== 'object') return [];
      
      // Verificar se tem video_library diretamente
      ...
      for (const [key, value] of Object.entries(obj)) {
        if (value && typeof value === 'object') {
          // Se valor é string, tentar fazer parse
    ...
    if (!videoLibrary || videoLibrary.length === 0) {
      ...
      console.log('🔍 [VIDEO-LIBRARY] Advanced settings estrutura completa:', 
        Object.keys(advancedSettings).join(', '));
      return null;
    }
    ...
    // Buscar vídeo por trigger exato
    ...
    console.log('🔍 [VIDEO-LIBRARY] Debug matching DETALHADO:', {
      ...
      triggersDisponiveis: videoLibrary.map(item => ({ 
        trigger: item.trigger, 
        name: item.name,
        triggerLower: item.trigger?.toLowerCase(),
        match: item.trigger?.toLowerCase() === normalizedSearchTrigger
    ...
    if (!video) {
      ...
      // Sugerir triggers similares
      const similarTriggers = videoLibrary
        .filter(item => item.trigger.toLowerCase().includes(normalizedSearchTrigger.substring(0, 3)))
        .map(item => item.trigger)
        .slice(0, 3);
    ...
    // Verificar se existe videoBase64
...
async function sendLibraryVideoMessage(
  ...
  try {
    ...
    // 2. CRIAR FORMDATA (IGUAL IMAGEM MAS MEDIATYPE=VIDEO)
    const timestamp = Date.now();
